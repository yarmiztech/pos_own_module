...
class BankFeesStatement(models.Model):
    ...
    def action_credit_statement(self):
        ...
        if bank_fee_config:
            ...
            raise UserError(_('Please Create Bank Fee reconciliation model under accounting setting'))
        if bank_fee_config:
            tax_list = []
        ...
        for move_l in bank_stmt.move_line_ids.filtered(lambda a: a.account_id.name == 'Bank Suspense Account'):
            print('Suspense Account')
            move_l.account_id = self.env['account.account'].search([('name', '=', 'Bank Charges')])
...
class TaxReturn(models.Model):
    ...
    create_date = fields.Date(string='Create Date', default=fields.Date.context_today, required=True)
    user_id = fields.Many2one('res.users', 'Users', index=True, ondelete='cascade', default=lambda self: self.env.user)
    tax_return_lines = fields.One2many('tax.return.line', 'tax_return_id')
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)
    note = fields.Text(string="Note")
    account_move = fields.Many2one('account.move')
    journal_id = fields.Many2one('account.journal', string="Payment Journal",domain=['|',('type', '=', 'bank'),('type', '=', 'cash')])
    difference_amount = fields.Float(string="Diff Amount")
    ...
    def action_view_journal(self):
        ...
        action_data['domain'] = [('id', 'in', self.account_move.ids)]
        return action_data
    ...
    def _onchange_to_dates(self):
          invoices = self.env['account.move'].search(
                [('invoice_date', '<=', self.to_date), ('invoice_date', '>=', self.from_date),
                 ('move_type', '=', 'out_invoice'), ('state', '=', 'posted'), ('company_id', '=', self.company_id.id)])

          list = []
          all = (0,0,{
              'source':'Sales',
              'total_value':sum(invoices.filtered(lambda a:a.amount_tax != 0).mapped('amount_total')),
          ...
          bills = self.env['account.move'].search(
              [('invoice_date', '<=', self.to_date), ('invoice_date', '>=', self.from_date),
               ('move_type', '=', 'in_invoice'), ('state', '=', 'posted'), ('company_id', '=', self.company_id.id)])

          po_all = (0, 0, {
              'source': 'Purchase',
              'total_value': sum(bills.filtered(lambda a:a.amount_tax != 0).mapped('amount_total')),
              'tax_amount': sum(bills.mapped('amount_tax'))

          })
          list.append(po_all)
          self.tax_return_lines =False
          self.tax_return_lines = list
          self.difference_amount = self.tax_return_lines.filtered(lambda a:a.source == 'Sales').tax_amount-self.tax_return_lines.filtered(lambda a:a.source == 'Purchase').tax_amount

    # def compute_difference_amount(self):
    #     for each in self:
    #         for line in each.tax_return_lines.filtered(lambda a:a.source == 'Sales'):
    #             line.tax_amount
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            ...
            #         'bank.fee.statement') or _('New')
            # else:
                vals['name'] = self.env['ir.sequence'].next_by_code('tax.return') or _('New')
        return super(TaxReturn, self).create(vals)
class TaxReturnLines(models.Model):
    _name = 'tax.return.line'

    tax_return_id = fields.Many2one('tax.return')
    source = fields.Char(string="Source")
    total_value = fields.Float(string="Total Sales")
